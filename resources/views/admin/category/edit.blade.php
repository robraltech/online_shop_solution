        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <div>
                @if($category->image)
                <img width="350" src="{{ asset('uploads/category/' . $category->image) }}"
                  alt="{{ $category->name }}"
                  class="img-thumbnail"
                  style="width: 350px;">
                @endif
  ...
  $(document).ready(function() {
    $("#categoryForm").submit(function(e) {
      ...
      $.ajax({
        ...
        dataType: "json",
        beforeSend: function() {
          $("#error-name, #error-slug").html("");
          $("#name, #slug").removeClass("is-invalid");
        },
        success: function(response) {
          if (response.status === true) {
            alert(response.message);
            window.location.href = "#"; // Redirect to categories list
          }
        },
        error: function(xhr) {
          if (xhr.status === 422) {
    ...
    // Auto-generate slug when name is changed
